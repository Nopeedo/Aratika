...
await pool(bills, 6, async (b) => {
  try {
    const d = await jget(`${API}/Bill/${b.id}`)
    const m = (d.Members && d.Members[0]) || null
    b.member = m ? m.DisplayName : null
    // Bill text on legislation.govt.nz (the official drafted version).
    b.legislationUrl = d.BillLegislationUrl || null
    // Select-committee submissions. This is the one moment a member of the
    // public can actually act on a bill, so capture whether submissions were
    // called and when they close — a stage change alone doesn't tell anyone
...
console.log('Member-in-charge, links and submission info resolved.')
...
const tracker = bills
  ...
  .map((b) => ({
    slug: b.slug, title: b.title, number: b.number, type: b.type, status: b.status,
    category: b.category, committee: b.committee, member: b.member, date: b.date,
    // Every bill links to its exact official page — a civic site that asks people
    // to trust it has to let them verify each claim at source, not just link the
    // Parliament homepage.
...
export interface Bill54 {
  ...
  /** The bill's own page on bills.parliament.nz — stages, documents, submissions. */
  officialUrl: string
  /** The drafted bill text on legislation.govt.nz, when published. */
  legislationUrl: string | null
  /** Present only while a select committee has called for public submissions. */
