...
// `next` is attacker-controllable and gets concatenated onto our origin. The
// test lives in src/lib/auth/safe-next.ts, shared with /login and /register.
...
export async function GET(request: Request) {
  ...
  const next = isRecovery ? '/reset-password' : safeNext(searchParams.get('next'), { origin })
