  ...
  return (
    <div>
        ...
        {options.map((o) => (
          <div key={o.kind + o.refId} style={{ display: 'flex', alignItems: 'center', gap: 10, border: `1px solid ${BORDER}`, borderLeft: `4px solid ${o.accent || JADE}`, borderRadius: 12, padding: '10px 12px', background: '#fff' }}>
            <span style={{ flex: 1, minWidth: 0 }}>
              <span style={{ display: 'block', fontSize: 14, fontWeight: 800, color: INK, fontFamily: MANROPE }}>{o.label}</span>
              {o.sublabel && <span style={{ display: 'block', fontSize: 11.5, color: TERTIARY, fontFamily: MANROPE }}>{o.sublabel}</span>}
            </span>
      ...
      <div style={{ marginTop: 16, padding: '14px 16px', borderRadius: 14, background: n > 0 ? '#ecfdf5' : '#f8fafc', border: `1px solid ${n > 0 ? '#a7f3d0' : BORDER}`, transition: 'background .2s' }}>
        {loading ? (
          <span style={{ fontSize: 13, color: TERTIARY, fontFamily: MANROPE }}>Loading your tracked items…</span>
        ) : n === 0 ? (
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13.5, color: SECONDARY, fontFamily: MANROPE }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 800, color: INK, fontFamily: MANROPE }}>You&apos;re tracking {n} {n === 1 ? 'thing' : 'things'}</span>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 12 }}>
              {bookmarks.map((b) => (
                <span key={b.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12.5, fontWeight: 700, color: INK, background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 999, padding: '5px 11px', fontFamily: MANROPE }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 16px', alignItems: 'center' }}>
              {user ? (
                <Link href="/dashboard" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, fontWeight: 800, color: '#fff', background: JADE, borderRadius: 9, padding: '8px 14px', textDecoration: 'none', fontFamily: MANROPE }}>
                  Open your command centre <ArrowRight style={{ width: 14, height: 14 }} />
                </Link>
              ) : (
                <>
                  <Link href="/register" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, fontWeight: 800, color: '#fff', background: JADE, borderRadius: 9, padding: '8px 14px', textDecoration: 'none', fontFamily: MANROPE }}>
                    Save these — sign up free <ArrowRight style={{ width: 14, height: 14 }} />
                  </Link>
                  <span style={{ fontSize: 12.5, color: SECONDARY, fontFamily: MANROPE }}>They&apos;re saved on this device meanwhile.</span>
                </>
              )}
            </div>
          </div>
