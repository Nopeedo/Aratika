    ...
    <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
      <div style={{ padding: '14px 0', borderBottom: `1px solid ${BORDER}`, display: 'flex', flexDirection: 'column', gap: 5, cursor: 'pointer' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 800, color: INK, fontFamily: MANROPE }}>{item.name}</span>
          <ArrowUpRight style={{ width: 13, height: 13, color: TERTIARY, marginLeft: 'auto' }} />
        </div>
        <p style={{ fontSize: 13, fontWeight: 500, color: SECONDARY, fontFamily: MANROPE, lineHeight: 1.5, margin: 0 }}>{item.desc}</p>
      </div>
    </a>
  )
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 48, alignItems: 'start' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: INK, fontFamily: MANROPE, margin: 0 }}>Follow the news</h2>
                <SectionDivider type="official" label="At the source" />
              </div>
              <Link href="/news" style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 800, color: INK, fontFamily: MANROPE, textDecoration: 'none' }}>
                All news <ArrowRight style={{ width: 13, height: 13 }} />
              </Link>
            </div>
            <p style={{ fontSize: 12, color: TERTIARY, fontFamily: MANROPE, marginBottom: 4, marginTop: 0 }}>
          ...
          <div>
            <div style={{ marginBottom: 16 }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 800, color: INK, fontFamily: MANROPE, margin: '10px 0 4px' }}>What matters most to you?</h2>
              <p style={{ fontSize: 12, color: TERTIARY, fontFamily: MANROPE, margin: 0 }}>Answer a few quick questions — we&apos;ll show you where parties stand on it.</p>
            </div>
            <div style={{ background: 'linear-gradient(155deg,#0f9152,#0c0e12)', borderRadius: 20, padding: '24px 22px', boxShadow: '0 6px 24px rgba(12,14,18,.10)' }}>
              ...
              <div style={{ fontSize: 17, fontWeight: 800, color: '#fff', fontFamily: MANROPE, marginBottom: 8 }}>Find your issues</div>
              <p style={{ fontSize: 13.5, fontWeight: 500, lineHeight: 1.55, margin: '0 0 18px', color: 'rgba(255,255,255,.8)', fontFamily: MANROPE }}>
                Pick what you care about and how much you already know — and Aratika will pitch everything at your level. No account needed.
              </p>
              <Link href="/start" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '11px 0', borderRadius: 11, background: '#fff', color: '#0c0e12', fontSize: 14, fontWeight: 800, textDecoration: 'none', fontFamily: MANROPE }}>
                <Compass style={{ width: 16, height: 16 }} /> Start the walkthrough
              </Link>
            </div>
            <p style={{ marginTop: 12, fontSize: 11, color: TERTIARY, fontFamily: MANROPE }}>
